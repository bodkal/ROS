...
class turtlebot_move():
    def __init__(self):
        rospy.init_node('turtlebot_pid', anonymous=False)
        ...
        self.read_goal = rospy.Subscriber("goal_node", Pose2D, self.read_goal_callback)

        self.vel_pub = rospy.Publisher('cmd_vel', Twist, queue_size=10)
        self.in_place = rospy.Publisher('in_place', Bool, queue_size=10)
...
#                angular = angular/abs(angular)*0.2
    ...
    def read_goal_callback(self, pos_msg):


        if(abs(pos_msg.x-self.x)>0.05 or abs(pos_msg.y-self.y)>0.05):
            rospy.loginfo("move to: x=" + str(pos_msg.x) + ";  y=" + str(pos_msg.y))
            self.move_to_point(pos_msg.x,pos_msg.y)
        else:
            self.in_place.publish(True)
